...
const PRICE_PER_UNIT = 100; const DAILY_LIMIT = 10;
...
module.exports = {
  ...
  async execute(interaction){
    ...
    if (sub === 'panneau'){
      const menu = new StringSelectMenuBuilder().setCustomId('illegal_select_item')
        .setPlaceholder('Choisis un item')
        .addOptions(ITEMS.map(i => ({ label:i.label, value:i.key, description:`${PRICE_PER_UNIT}$ / unité` })));
      const row = new ActionRowBuilder().addComponents(menu);
      const e = new EmbedBuilder().setColor(C.primary).setTitle('🏴 Vendeur illégal — Approvisionnement')
        .setDescription(`• Commande via **/commande-illegale acheter** (item, quantité 1–10, propriété_id)\n• **${PRICE_PER_UNIT}$** / unité — **${DAILY_LIMIT}** max/jour/item\n• Dépôt **instantané** dans le stockage de votre site (type correspondant).`)
    ...
    if (sub === 'acheter'){
      ...
      if ((econ.current.liquid||0) < cost) return interaction.reply({ embeds:[ new EmbedBuilder().setColor(C.danger).setDescription(`Fonds insuffisants : il faut **${cost}$** en liquide.`)]});
      econ.current.liquid -= cost; setUser(interaction.guildId, interaction.user.id, (u)=>{ u.frozen=econ.frozen; u.current=econ.current; u.business=econ.business; });
      ...
      const e = new EmbedBuilder().setColor(C.success).setTitle('✅ Commande validée')
        .setDescription(`Item: **${def.label}** (*${itemKey}*)\nQuantité: **${qty}** — Total: **${cost}$**\nSite: **${prop.name}** *(type ${ptype})* → dépôt **instantané**.`);
